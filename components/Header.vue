...
<template>
  <header class="py-2 px-4 md:px-16 flex justify-between bg-gradient-to-r from-[#000000] to-[#252525]">
    <NuxtLink to="/" class="hover:scale-105 transition-all"><img :src="siteData.siteLogo" alt="NGMaker" width="217"></NuxtLink>
    <button @click="openMenu" id="menuBtn" class="block md:hidden bg-gradient-to-r from-[#f0ae21] to-[#feb201] h-14 w-14 rounded-full text-white"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="35" height="35" class="mx-auto" fill="#fff"><title>Menu</title><path d="M3,6H21V8H3V6M3,11H21V13H3V11M3,16H21V18H3V16Z" /></svg></button>
    <nav id="fullMenu" class="hidden md:flex">
        ...
        <ul id="menuList" class="flex gap-1 items-center">
            <li v-for="item in siteData.navigation" :key="item.url" class="p-3 text-lg text-slate-200 align-middle">
                <NuxtLink :to="item.url" class="hover:text-[#feb201] hover:scale-105 transition-all font-bold">{{ item.name }}</NuxtLink>
            </li>
        </ul>
    </nav>
</header>
